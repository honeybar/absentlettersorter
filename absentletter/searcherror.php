...
?>
        ...
        <h3>Please specify the date for search! </h3>
        <!-- end search engine -->
		...
		$search=mysql_query("SELECT DISTINCT absent_letter.date AS date
				...
				ORDER by date DESC"); 
				// search for absentees where the students belong to the user's class
				
		$total=mysql_num_rows($search); // find the total no of records found 

		if($total<1) // if no record found...
		{
			echo("<p>No data available</p>"); // output not available
		}
    	...
    	<?php
		$dataperpage=5; // determine the no of records displayed per page
		...
		$search=mysql_query("SELECT DISTINCT absent_letter.date AS date
							...
							ORDER by date DESC LIMIT $offset, $dataperpage");
							// control the no of records displayed per page
							
    	while($displaydate=mysql_fetch_array($search))
		{
			
			$check=mysql_query("SELECT absent_letter.date AS date FROM absent_letter_sorter.absent_letter, school_db.studentlisttbl,
			 					school_db.classtbl
								WHERE studentlisttbl.Student_ID = absent_letter.Student_ID
								AND studentlisttbl.Class_ID = classtbl.Class_ID  
								AND studentlisttbl.Class_ID='$classid' AND date='$displaydate[date]'
								AND letters_given='0'"); 
